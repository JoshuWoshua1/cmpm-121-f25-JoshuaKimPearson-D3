...
- **2025-11-14** — Changed basis to use the true origin (0, 0) instead of the classroom. Removed old futureproofing in order to properly implement a dynamic map view that can also have the farming bug. Memoryless cells have also been implemented by avoiding restoring points with `cellContents`. Fixed a bug that popped up where I couldn't craft properly or pick up tokens. My old system with `cellContents` was conflicting with the memoryless cell implementation. Now the farming glitch works as well by clearing the `cellContents` of cells off screen.

- **2025-11-15** — Created segment in `PLAN.md` for D3.c. Removed my backwards fix for farming glitch and now `cellContents` is not cleared once cells are off screen. Finished memento system so that unodified cells are never stored in memory while modified ones are, preserving a lot more memory space. Created A debug menu for checking how many cells are being stored in memory. D3.c complete Debug code removed, but menu left intact for future use.

- **2025-11-20** — Changed movement to follow the Facade Architecture. Now button movement and geolocation can be toggled.

- **2025-11-23** — Completed D3.d implementation: Enhanced geolocation system with user-friendly permission requests, error handling (permission denied, timeout, position unavailable), and visual status indicator showing GPS accuracy in meters. Implemented full state persistence using localStorage: game state (playerToken, playerLatLng, cellContents Map) now serializes to JSON and auto-saves after every action (collection, crafting, placement, movement). Added load-on-startup feature that restores saved game state or starts fresh if no save exists. Created "New Game" button with confirmation dialog that resets all state (inventory, position, cells, localStorage) and properly reinitializes the movement facade. Movement mode preference is now persisted to localStorage and restored on page reload. Game now fully supports playing across real-world space (via geolocation) and time (via persistence)
