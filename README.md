...
- **2025-11-15** — Created segment in `PLAN.md` for D3.c. Removed my backwards fix for farming glitch and now `cellContents` is not cleared once cells are off screen. Finished memento system so that unodified cells are never stored in memory while modified ones are, preserving a lot more memory space.
